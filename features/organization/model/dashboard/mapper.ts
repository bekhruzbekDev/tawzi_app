import { AlertStatItem, DashboardData, DashboardStats, Finance, FinanceChartData, MonthlyStats } from "./types";


export const mapDashboardDataToStats = (data:MonthlyStats|null,): DashboardStats[] => {
...
export const  AlertStatsMapper = (data:DashboardData|null,hasBilling:boolean): AlertStatItem[] => {
    ...
    return [
        {
            ...
            color: "red",
            text: `${data?.debt?.value??0} ta` ,price:data?.debt?.price??"",label:"Qarzdorlik :",
        },
        {
            value: data?.paid?.value??0,
            color: "green",
            text: `${data?.paid?.value??0} ta` ,price:data?.paid?.price??"",label:"To'langan :",
        },
        {
            value: data?.warned?.value??0,
            color: "orange",
            text: `${data?.warned?.value??0} ta` ,price:data?.warned?.price??"",label:"Ogohlantirilgan :",
        },
    ]
}